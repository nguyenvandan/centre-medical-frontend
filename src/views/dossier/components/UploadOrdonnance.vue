  ...
  <div :class="docContainer">
    ...
    <div class="flex w-full mt-0 mb-0 sm:px-3 items-center">
      ...
      <div class="w-10/12 lg:w-8/12 flex justify-center">
        <h2 class="text-left sm:text-center text-white bg-twarning py-2 px-5"><b>Je joins mon ordonnance</b></h2>
      </div>
    </div>
    ...
    <div v-for="(doc, idx) in documentToUpload" :key="idx" :class="[docClass, docLayoutClass]">
      ...
      <div class="w-full sm:w-4/12 lg:w-4/12 py-2 px-3">
        <a :href="$getFileLink(doc.file) || ''" target="_blank">{{ getFilename(doc.file) || '' }}</a>
      </div>
    </div>
